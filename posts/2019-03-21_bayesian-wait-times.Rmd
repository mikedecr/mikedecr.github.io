...
Suppose there is a true wait time $\omega$ for voter $i$ in state $s$. We assume $\omega_{i}$ as a Poisson draw from the distribution of wait times in state $s$,
\begin{align}
  \omega_{i} &\sim \mathrm{Poisson}(\lambda_{s[i]}),
\end{align}
where $\lambda_{s}$ is the mean wait time in state $s$.

Wait times are measured in surveys using discrete categories to represent a range of wait times. Let $y_{i}$ be the discrete measure of wait time for voter $i$. If there are $K$ total categories, then the probability that $y_{i} = k$ is given by a difference in cumulative Poisson functions.
\begin{align}
  \mathrm{Pr}(y_{i} = k) &= \displaystyle e^{-\lambda_{s} }\sum _{r=k^{-}}^{k^{+}}{\frac {\lambda_{s}^{r}}{r!}}
\end{align}
...
The observed wait time is measured in ordinal bins. As long as $\omega_i$ is a random variable from the Poisson process, then its falling into a particular bin can be represented as a categorical outcome.
...
And $\omega_{i}$ is a Poisson draw from the state mean wait.
\begin{align}
  \omega_{i} &\sim \mathrm{Poisson}(\lambda_{s[i]}),
\end{align}
