...
As just mentioned, R is a flexible tool for doing statistical computing. More than that, it has a *diversity of methods* that users can use to accomplish similar tasks. That is, there are many ways to do the same thing. 

As a *teacher* of R, this makes things difficult. As an experienced user, instructors see the diversity of methods as a benefit. But for new R users, this diversity can be daunting and confusing. When you're learning a new language, sometimes you just want to be told what works. 

Since we have a very short time to cover as much quality R as we can, we have to make some choices about where to place our emphasis. Here is the approach we will take. I confess to having my thinking heaviliy influenced by some blog posts by David Robinson about teaching R (including but not limited to [here](http://varianceexplained.org/r/teach-tidyverse/)).

- Rather than create exercises where I force you to slog through difficult problems using underpowered tools (for the purpose of "getting you to appreciate the finer points of the language" or something), we try to jump quickly into the stuff that I believe will be *most useful to you in the future*. That is, we'll focus more on what works rather than painstakingly demonstrating the differences between old and new ways of doing things.
...
- We won't painstakingly discuss how multiple routes lead to the same end result, *except* in cases where you will use multiple routes (when we do recoding, for example). Instead, we'll *simplify the problem* by explaining the tools you are most likely to use.
...
- [First is an overview of some basic R content.](811/811-basics) This covers some introductory guidance for installing R and running simple commands. ***Review this page on your own before the first in-class R lesson.*** 
- [Day 1 covers data manipulation,](811/811-data) focusing specifically on useful tools in the suite of R packages commonly known as the `tidyverse`.
- [Day 2 covers graphics,](811/811-graphics) with a particular focus on `ggplot2`.
- [Day 3 covers statistical analysis,](811/811-analysis) including estimating models, generating model output, and incorporating statistical results into a social science workflow.

In-class lectures will be guided and motivated walkthroughs of code examples. These will be most effective if you review the online notes *before* class, as we cannot get through everything in class (moreover, it would be a waste of face-time). 

Take-home exercises will reinforce material from the online notes. These exercises are mandatory. I will release "solutions" and am happy to review your code and work through any issues with you. 

The dataset we will use is the American National Election Study, which is a long-running academic survey of U.S. voting and public opinion. Specifically, we will work with the "[cumulative data file](http://electionstudies.org/studypages/anes_timeseries_cdf/anes_timeseries_cdf.htm)," which contains interviews from survey respondents for presidential and midterm campaign seasons since 1948. Make an account on [electionstudies.org](http://electionstudies.org/) and download the cumulative data file before the first in-class lesson. Make sure the download includes the codebook. 
