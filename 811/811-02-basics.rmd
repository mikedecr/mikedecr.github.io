...
Indexing works with higher-dimensional data as well. Here are some two-dimensional arrays of numbers. 
...
Logic as a concept in programming is less daunting than it sounds. At it's core, what we're doing is dealing with is statements that evaluate to either `TRUE` or `FALSE`. 
...
The statement `g > 5` is a "logical" vector, meaning that its values are either `TRUE` or `FALSE`, which are keywords in R (and must be capital). R asks if `g` is greater than 5, and since `g` is a vector, R returns a vector of logicals with each element corresponding to the original elements in `g`. 
