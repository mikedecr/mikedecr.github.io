...
You can coerce data from type to another, and they follow some intuitive rules (with certain trade-offs).
...
By placing `NA` inside a numeric vector, what we've told R is that we know that the missing data is numeric, we just don't know what the number is. If the value of the number data, some functions fail. For example, we don't know the mean of `h`. Try the following.
