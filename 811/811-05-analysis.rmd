...
Some other tools create these sorts of plots for you. Some folks enjoy using [`sjPlot`](http://www.strengejacke.de/sjPlot/sjp.lm/), but I have never bothered to use it. I normally don't like packages to make plots for me. I like packages to create things that I can plot how I want. Speaking for myself, though.
...
- In a linear model: $x$ is assumed to have a linear effect on $y$. Coefficients describe how $x$ impacts $y$.
- In a GLM: $x$ has a linear impact on a *transformation* of $y$, which makes $x$ non-linearly related to $y$. Coefficients describe how $x$ impacts the transformed $y$.
...
The transformation of $y$ is called a *link function*. We call these "generalized linear models" because there *is* a linear component, but it's a linear relationship between $x$ and $\mathrm{link}(y)$. And it's *generalized* because a linear model fits into this framework as well, but the link function is simply $1 \times y$.
