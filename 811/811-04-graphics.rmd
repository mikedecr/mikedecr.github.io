...
- This lesson makes heavy use of pipe chains, which string many operations together in one block of code. I will not painstakingly describe every step of every step chain, so here is what I advise you do. In order to understand each step of a pipe chain, run it command by command. You can see the results of each step and how they feed into subsequent steps.
- We will not talk about the `qplot` function in `ggplot2`. Some `ggplot` lessons start here because `qplot` is like a midway point between base and ggplot graphics. But since we won't cover base graphics at all, I see no point in talking about `qplot`. You won't use it anyway.
...
Notice the use of `mutate_all()`. This function is like `mutate`, but it applies a function to every column in the data (in this case, the `remove_labels()` function). If you're familiar with R's `apply()` functions or functional programming in general, this is a similar idea. We'll cover those things at the end of the next lesson.
...
It is not necessary to consciously memorize the theoretical components of the grammar of graphics. In fact, thinking too hard about it can be confusing at first. You will get an intuition for them as you use ggplot without being able to recite the tenets, but for the sake of exposition, here are the principles of the grammer. There is *no need* to memorize this list:

- **data**, which are visualized
- **aesthetic mappings** that describe how variables are mapped to aesthetic attributes (such as plot axes, colors)
- **geoms**, geometric representations of the data, such as lines, points, etc., that control how the cells in your data are presented on the page
- **scales** which modify how aesthetic mappings are presented. These are used to change colors, axes, and so on.
- a **coordinate system** that defines the plane on which you're plotting. Most plots use a Cartesian plane but could use less conventional planes (e.g. polar coordinates).
- a **faceting system** for breaking the data into subsets. Subsets are displayed in separate plot panels
...
Consult the codebook for information on the "feeling thermometer." You'll notice that values 97-100 are represented with the number 97 in the codebook, while 98 and 99 are special codes for "don't know" and missing responses. I have no idea why this decision was made, but we have to work with it. We'll want to recode these as `NA`. 

Once we do that, We'll also take the difference between the thermometer scores and create a *relative* thermometer score, where positive values indicate that an individual feels more warmly to the Republican candidate than to the Democratic candidate.
...
```{r}
# add data to the plot as points
ggplot(data = anes, aes(x = pid7, y = reltherm_cand)) +
  geom_point()
```
...
```{r}
# jitter the points IN THE PLOT, not in the underlying data
# modify the jitter amount (width), point shape, and opacity
ggplot(data = anes, aes(x = pid7, y = reltherm_cand)) +
...
```{r}
# add a fit line from a linear regression of y on x
ggplot(data = anes, aes(x = pid7, y = reltherm_cand)) +
  geom_jitter(width = 0.2, shape = 1, alpha = 0.3) +
...
```{r}
# color the fit line according by gender
...
```{r}
# if color is declared in ggplot(), it is automatically added to geoms
# (where applicable)
...
```{r}
# global colors can overridden in geom functions
ggplot(data = anes, aes(x = pid7, y = reltherm_cand, color = gender)) +
...
```{r}
# axis labels can be specified separately from breaks
ggplot(data = anes, aes(x = pid7, y = reltherm_cand)) +
  geom_jitter(width = 0.2, 
              shape = 1, alpha = 0.3, size = 0.25, 
              color = "slategray") +
...
```{r}
# add aesthetic labels using labs()
ggplot(data = anes, aes(x = pid7, y = reltherm_cand)) +
  geom_jitter(width = 0.2, 
              shape = 1, alpha = 0.3, size = 0.25, 
              color = "slategray") +
...
# can specify changes to the coordinates when you do this as well
ggplot(data = anes, aes(x = pid7, y = reltherm_cand)) +
  geom_jitter(width = 0.2, 
              shape = 1, alpha = 0.3, size = 0.25, 
              color = "slategray") +
...
```{r}
# separate panels for each gender
# wrapping: if there are lots of panels, the grid is wrapped from one end to the next
# use the ~ symbol (necessary for technical reasons)
...
```{r}
# grid panels, set row and column variables
# ~ symbol still necessary
ggplot(pid_gendergap, aes(x = cycle, y = prop, color = party)) +
...
```{r}
# use a period . to say "don't do rows" or "don't do columns"
ggplot(pid_gendergap, aes(x = cycle, y = prop, color = party)) +
  facet_grid(. ~ gender) +
  geom_line()
```
...
```{r}
# geom_ribbon draws a polygon using x aesthetic and ymin and ymax
# geom_polygon also exists but requires more specificity
ggplot(pid_gendergap, aes(x = cycle, y = prop, color = party)) +
  facet_grid(. ~ gender) +
  geom_line() +
  geom_point() +
...
```{r}
# drop ribbon color with color = NA
ggplot(pid_gendergap, 
       aes(x = cycle, y = prop, color = party, fill = party)) +
  facet_grid(. ~ gender) +
  geom_line() +
...
```{r}
# more changes to make it prettier
ggplot(pid_gendergap, 
       aes(x = cycle, y = prop, color = party, fill = party)) +
  facet_grid(. ~ gender) +
  geom_line() +
...
There is also a `theme()` function, which allows you to change individual details of the theme without changing the entire theme. Here we will suppress the "minor" grid lines for the y axis only.
...
Saving this graphic is easy with the `ggsave` function. You generally want to save graphic as vector images in order to scale (that is, increase the size of) a graphic without any pixelation. PDF is usually good for papers and presentations.
...
# pipe into the ggplot function (can do that!)
anes %>%
  filter(!is.na(party)) %>% 
...
```{r}
# make histogram bars non-stacked
anes %>%
  filter(!is.na(party)) %>% 
  ggplot(aes(x = reltherm_cand)) +
    geom_histogram(aes(color = party,
...
- If you can use aesthetic features *besides* color to distinguish groups, that would be ideal. Some forms of color-blindness are remarkably common, and at any rate, journals may not allow your article to print in color. 
